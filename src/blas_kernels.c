...
void gemm_offset_gpu_strided_batched(
    int TA, int TB, int M, int N, int K,
    float alpha,
    cl_mem_ext A_gpu, size_t offset_A, int lda, size_t a_stride,
    cl_mem_ext B_gpu, size_t offset_B, int ldb, size_t b_stride,
    float beta,
    cl_mem_ext C_gpu, size_t offset_C, int ldc, size_t c_stride,
    int batch_count)
{
    ...
    clErr = CLBlastSgemmStridedBatched(CLBlastLayoutRowMajor,
                         (TA ? CLBlastTransposeYes : CLBlastTransposeNo),
                         (TB ? CLBlastTransposeYes : CLBlastTransposeNo),
                         M, N, K,
                         alpha,
                         A_gpu.mem, offset_A, lda, a_stride,
                         B_gpu.mem, offset_B, ldb, b_stride,
                         beta,
                         C_gpu.mem, offset_C, ldc, c_stride,
                         batch_count, &que, NULL);
    // if(gemm_round++ % 4 == 0)
    // clFinish(que);
...
//     cl_command_queue que = opencl_queues[opencl_device_id_t];
//     // Map clblasRowMajor to CLBlast layout
//     CLBlastLayout layout = CLBlastLayoutRowMajor;
//     CLBlastTranspose a_transpose = TA ? CLBlastTransposeYes : CLBlastTransposeNo;
//     CLBlastTranspose b_transpose = TB ? CLBlastTransposeYes : CLBlastTransposeNo;
//     clErr = CLBlastSgemm(layout,                // Row-major layout
...
void gemm_offset_gpu(
    ...
    // int tuning = (opencl_device_id_t == 0) ? 8 : 16;
    int tuning =8;
    dim3 dimGridG1;
