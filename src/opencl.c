...
void opencl_set_device(int n)
{
    ...
    opencl_device_ct_t = number;
    opencl_device_id_t = n;
    printf("oopencl_set_device pencl_device_id_t value: %d\n", opencl_device_id_t);
}
...
void opencl_load_buffer(const char *buffer, const size_t size, cl_program *output)
{
    ...
    *output = clCreateProgramWithSource(opencl_context, 1,
    ...
    cl_device_id *opencl_devices0;
    // opencl_devices0 = (cl_device_id *)calloc((cl_uint)(number-1), sizeof(cl_device_id));
    // for (int i = 1; i <= 2; ++i) // 获取两个设备 cpu+gpu
    // {
    //     opencl_devices0[i-1] = opencl_devices[i];
    // }
...
{   
    ...
    // opencl_devices = (cl_device_id *)calloc((cl_uint)opencl_device_ct_t, sizeof(cl_device_id));
    // opencl_queues = (cl_command_queue *)calloc((cl_uint)opencl_device_ct_t, sizeof(cl_command_queue));
    ...
    char *value;
    size_t valueSize;
    cl_uint maxComputeUnits, max_subs;
    ...
    for (i = 0; i < 2; ++i) // 获取两个设备 cpu+gpu
    {   if(i==1){opencl_devices[i+1] = devices[i];}
        else{
        opencl_devices[i] = devices[i];}
    }

    cl_props[1] = (cl_context_properties)clPlatform;
    // 这里不确定是只创建一个上下文，还是根据子设备的数量创建多个上下文，可以试一下。
    ...
    cl_command_queue_properties queues_properties = CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE;
    int d;
    for (d = 0; d < 2; ++d)
    {
        if(d==1){opencl_device_id_t = d+1;}
        else{
        ...
        clGetDeviceInfo(opencl_devices[opencl_device_id_t], CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(maxComputeUnits), &maxComputeUnits, NULL);
        printf("Device parallel compute units: %d\n", maxComputeUnits);
        clGetDeviceInfo(opencl_devices[opencl_device_id_t], CL_DEVICE_PARTITION_AFFINITY_DOMAIN, sizeof(affinity_domain), &affinity_domain, NULL);
        printf("Device has affinity domain: %x\n", (unsigned int)affinity_domain);
        ...
        // cl_device_partition_property properties[] = {CL_DEVICE_PARTITION_BY_COUNTS, 23, 5, CL_DEVICE_PARTITION_BY_COUNTS_LIST_END, 0};
        cl_device_partition_property properties[] = {CL_DEVICE_PARTITION_EQUALLY, maxComputeUnits / 2, 0};
        // cl_device_partition_property properties[] = { CL_DEVICE_PARTITION_BY_AFFINITY_DOMAIN, CL_DEVICE_AFFINITY_DOMAIN_NUMA, 0};

        clErr = clCreateSubDevices(opencl_devices[opencl_device_id_t], properties, 0, NULL, &num_subdevices);
        // opencl_devices = (cl_device_id *) calloc((cl_uint)ngpus, sizeof(cl_device_id));
        cl_device_id *opencl_sub_devices;
        opencl_sub_devices = (cl_device_id *)malloc(num_subdevices * sizeof(cl_device_id));
        printf("num_subdevices = %d\n", num_subdevices);
        clErr = clCreateSubDevices(opencl_devices[opencl_device_id_t], properties, num_subdevices, opencl_sub_devices, NULL);
        if (clErr != CL_SUCCESS)
        {
            printf("Error: Failed to createSubdevice!\n");
        }
        int i;
        for (i = 0; i < num_subdevices; ++i)
        {
            opencl_devices[opencl_device_index++] = opencl_sub_devices[i];
        }
        free(opencl_sub_devices);
    }
        // 统一创建上下文（包含所有子设备）
        opencl_context = clCreateContext(cl_props, opencl_device_index, opencl_devices, NULL, NULL, &clErr);
        if (clErr != CL_SUCCESS)
        {
            printf("opencl_init: Could not create context.\n");
            exit(-1);
        ...
        // 创建队列并打印每个子设备信息
        for (int j = 0; j < opencl_device_index; j++)
        {
            opencl_device_id_t = j;
            ...
            clGetDeviceInfo(opencl_devices[j], CL_DEVICE_PARTITION_MAX_SUB_DEVICES, sizeof(max_subs), &max_subs, NULL);
            printf(" %d.%d max_sub_deveces: %d\n", j + 1, 4, max_subs);

            clGetDeviceInfo(opencl_devices[j], CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(maxComputeUnits), &maxComputeUnits, NULL);
            printf(" %d.%d Parallel compute units: %d\n", j + 1, 5, maxComputeUnits);

            opencl_queues[j] = clCreateCommandQueue(opencl_context, opencl_devices[j], CL_FALSE, &clErr);
        
        // opencl_context = clCreateContext(cl_props, number, opencl_devices, NULL, NULL, &clErr);
        // // opencl_queues = (cl_command_queue *)malloc(num_subdevices * sizeof(cl_command_queue));
...
void opencl_deinit(int *gpus, int ngpus)
{
    ...
    for (a = 0; a < ngpus; ++a)
    {
        ...
        xim2col_kernel_release();
        maxpool_kernel_release();
        gemm_kernel_release();
        xgemm_kernel_release();
...
void opencl_kernel(cl_kernel kernel, const dim2 globalItemSize, const int argc, ...)
{
    ...
    cl_device_id device;
    clGetCommandQueueInfo(que, CL_QUEUE_DEVICE, sizeof(cl_device_id), &device, NULL);
    cl_uint max_subs;
    clGetDeviceInfo(device, CL_DEVICE_PARTITION_MAX_SUB_DEVICES, sizeof(max_subs), &max_subs, NULL);
    // printf(" %d.%d max_sub_deveces: %d\n", j+1, 4, max_subs);
    printf("%s\t%d\t max: %d\n", kernelName, (int)time_taken, max_subs);
