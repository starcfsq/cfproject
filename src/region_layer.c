...
void get_region_detections_batch(layer l, int w, int h, int netw, int neth, float thresh, int *map, float tree_thresh, int relative, detection *dets,int offset)
{
    int i,j,n,z;
    float *predictions = l.output;
    // float *predictions = l.output + offset*l.outputs;
    // if (l.batch == 2) {
    //     float *flip = l.output + l.outputs;
    //     for (j = 0; j < l.h; ++j) {
    //         for (i = 0; i < l.w/2; ++i) {
    ...
    for (i = 0; i < l.w*l.h; ++i){
        ...
        for(n = 0; n < l.n; ++n){
            ...
            int obj_index  = entry_index(l, offset, n*l.w*l.h + i, l.coords);
            int box_index  = entry_index(l, offset, n*l.w*l.h + i, 0);
            int mask_index = entry_index(l, offset, n*l.w*l.h + i, 4);
            float scale = l.background ? 1 : predictions[obj_index];
            dets[index].bbox = get_region_box(predictions, l.biases, n, box_index, col, row, l.w, l.h, l.w*l.h);
            dets[index].objectness = scale > thresh ? scale : 0;
            if(dets[index].mask){
            ...
            int class_index = entry_index(l, offset, n*l.w*l.h + i, l.coords + !l.background);
            if(l.softmax_tree){

                hierarchy_predictions(predictions + class_index, l.classes, l.softmax_tree, 0, l.w*l.h);
                if(map){
                    for(j = 0; j < 200; ++j){
                        int class_index = entry_index(l, offset, n*l.w*l.h + i, l.coords + 1 + map[j]);
                        float prob = scale*predictions[class_index];
                        dets[index].prob[j] = (prob > thresh) ? prob : 0;
                    }
                } else {
                ...
                if(dets[index].objectness){
                    for(j = 0; j < l.classes; ++j){
                        int class_index = entry_index(l, offset, n*l.w*l.h + i, l.coords + 1 + j);
                        float prob = scale*predictions[class_index];
                        dets[index].prob[j] = (prob > thresh) ? prob : 0;
                    }
                }
