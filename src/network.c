...
{
#ifdef GPU
	// printf("forward_network netp->gpu_index value: %d\n", netp->gpu_index);
	// netp->gpu_index = -1;
    if(netp->gpu_index >= 0){
    ...
    for(i = 0; i < net.n; ++i){
		...
		const char* layerName[] = { "CONVOLUTIONAL","DECONVOLUTIONAL","CONNECTED", "MAXPOOL", "SOFTMAX", "DETECTION", "DROPOUT", "CROP", "ROUTE", "COST", "NORMALIZATION", "AVGPOOL", "LOCAL", "SHORTCUT", "ACTIVE", "RNN", "GRU", "LSTM", "CRNN", "BATCHNORM", "NETWORK", "XNOR", "REGION", "YOLO", "YOLO4", "ISEG", "REORG", "UPSAMPLE", "LOGXENT", "L2NORM", "BLANK","YOLO7"};
...
float *network_predict_cpu(network *net, float *input)
{
    ...
    // 寮哄埗浣跨敤CPU杩涜鎺ㄧ悊
    net->gpu_index = -1; // 璁剧疆gpu_index涓?1锛屽己鍒朵娇鐢–PU杩涜璁＄畻
    forward_network(net);
...
int num_detections_batch(network *net, float thresh, int offset)
{
	int i;
	int s = 0;
	for(i = 0; i < net->n; ++i){
		...
		if(l.type == YOLO){
			// s += yolo_num_detections(l, thresh);
			s += yolo_num_detections_batch(l, thresh,offset);
		}
		if(l.type == YOLO4){
			s += yolo4_num_detections_batch(l, thresh,offset);
		}
		if(l.type == DETECTION || l.type == REGION){
			s += l.w*l.h*l.n;
		}
...
detection *make_network_boxes_batch(network *net, float thresh, int *num, int offset)
{
	layer l = net->layers[net->n - 1];
	int i;
	// int nboxes = num_detections(net, thresh);
	int nboxes = num_detections_batch(net, thresh,offset);
	if(num) *num = nboxes;
	detection *dets = (detection*)calloc(nboxes, sizeof(detection));
	for(i = 0; i < nboxes; ++i){
		dets[i].prob = (float*)calloc(l.classes, sizeof(float));
...
void fill_network_boxes_batch(network *net, int w, int h, float thresh, float hier, int *map, int relative, detection *dets, int offset)
{
	int j;
	for(j = 0; j < net->n; ++j){
		layer l = net->layers[j];
		if(l.type == YOLO){
			// int count = get_yolo_detections(l, w, h, net->w, net->h, thresh, map, relative, dets);
			int count = get_yolo_detections_batch(l, w, h, net->w, net->h, thresh, map, relative, dets, offset);
			dets += count;
		}
		if(l.type == YOLO4){
			// int count = get_yolo4_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, 1);
			int count = get_yolo4_detections_batch(l, w, h, net->w, net->h, thresh, map, relative, dets, 1,offset);
			dets += count;
		}
		if(l.type == REGION){
			// get_region_detections(l, w, h, net->w, net->h, thresh, map, hier, relative, dets);
			get_region_detections_batch(l, w, h, net->w, net->h, thresh, map, hier, relative, dets,offset);
			dets += l.w*l.h*l.n;
		}
		if(l.type == DETECTION){
			// get_detection_detections(l, w, h, thresh, dets);
			get_detection_detections_batch(l, w, h, thresh, dets,offset);
			dets += l.w*l.h*l.n;
		}
	}
}
...
detection *get_network_boxes_batch(network *net, int w, int h, float thresh, float hier, int *map, int relative, int *num, int offset)
{
	// detection *dets = make_network_boxes(net, thresh, num);
	detection *dets = make_network_boxes_batch(net, thresh, num, offset);
	// fill_network_boxes(net, w, h, thresh, hier, map, relative, dets);
	fill_network_boxes_batch(net, w, h, thresh, hier, map, relative, dets, offset);
	return dets;
}
...
{	
	...
	// printf("  CONVOLUTIONAL Time: %f seconds\n", CON_time);
	// CON_time=0;
