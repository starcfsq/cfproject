...
LDFLAGS+= -L/usr/local/lib -L/usr/lib/arm-linux-gnueabihf -lpocl -lOpenCL -lclblast -L/lib/aarch64-linux-gnu -lclBLAS
LDFLAGS+= -L/usr/lib
else
COMMON+= -DCL_TARGET_OPENCL_VERSION=120 -DARM
CFLAGS+= -DARM -I/usr/include/ -I/usr/local/include/
# LDFLAGS+= -L/usr/local/lib -L/usr/lib/arm-linux-gnueabihf -lOpenCL
LDFLAGS+= -L/usr/local/lib -L/usr/lib/arm-linux-gnueabihf -lpocl
LDFLAGS+= -L/usr/lib
endif
endif
...
# LDFLAGS+= -L/usr/lib/x86_64-linux-gnu/ -lOpenCL -lclBLAS -L/usr/local/lib -lclblast -L/usr/local/lib
LDFLAGS+= -L/usr/lib/x86_64-linux-gnu/ -lOpenCL  -lclBLAS -L/usr/local/lib -lpocl -lclblast
LDFLAGS+= -L/usr/lib64
endif
ifeq ($(NVIDIA), 1)
COMMON+= -DGPU -DOPENCL
CFLAGS+= -DGPU -DOPENCL -I/usr/include/ -I/usr/local/cuda/include/
# LDFLAGS+= -L/usr/local/cuda/lib64 -lOpenCL -L/usr/lib64 -lclBLAS -L/usr/local/lib -lclblast -L/usr/local/lib
LDFLAGS+= -L/usr/local/cuda/lib64 -lOpenCL -L/usr/lib64 -lclBLAS -L/usr/local/lib -lpocl -lclblast
LDFLAGS+= -L/usr/lib64
endif
endif
...
ifeq ($(GPU), 1) 
LDFLAGS+= -lstdc++
OBJ+=convolutional_kernels.o deconvolutional_kernels.o activation_kernels.o im2col_kernels.o col2im_kernels.o blas_kernels.o crop_layer_kernels.o dropout_layer_kernels.o maxpool_layer_kernels.o avgpool_layer_kernels.o xgemm.o
else
#LDFLAGS+= -lstdc++
OBJ+=cpu.o
endif
